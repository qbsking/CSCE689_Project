...
public:
	...
	void outputResults(ICBSNode *curr);
	void outputType(std::list<std::shared_ptr<Conflict>> &conflictList, std::vector<int> &results, int agent1, int agent2, std::shared_ptr<Conflict> chosen);
	void generate_inputs(ICBSNode &node, std::shared_ptr<Conflict> &conflict, std::vector<double> &inputs);
	int record_ones();
	void output_cost(int num);
