...
void ICBSSearch::generate_inputs(ICBSNode &node, std::shared_ptr<Conflict> &conflict, std::vector<double> &inputs)
{
	if (get<2>(*conflict) < 0)
	{
		inputs[0] = 1.0;
		inputs[3] = getRectangleTime(*conflict, paths, ml->cols);
	} else if (get<3>(*conflict) < 0)
	{
		inputs[1] = 1.0;
		inputs[3] = get<4>(*conflict);
	} else {
		inputs[2] = 1.0;
		inputs[3] = get<4>(*conflict);
	}
	inputs[3] = inputs[3]/10.0;
	inputs[4] = node.dup_Ncollisions;
	inputs[5] = static_cast<int>(node.depth);

	std::vector<int> temp_results;
	int id1 = get<0>(*conflict), id2 = get<1>(*conflict);
	int id1_c = 0, id2_c = 0;

	outputType(node.dup_rectSemiConf, temp_results, id1, id2, conflict);
	inputs[6] = temp_results[0];
	id1_c += temp_results[1];
	id2_c += temp_results[2];

	outputType(node.dup_rectNonConf, temp_results, id1, id2, conflict);
	inputs[7] = temp_results[0];
	id1_c += temp_results[1];
	id2_c += temp_results[2];

	outputType(node.dup_cardinalConf, temp_results, id1, id2, conflict);
	inputs[8] = temp_results[0];
	id1_c += temp_results[1];
	id2_c += temp_results[2];

	outputType(node.dup_semiConf, temp_results, id1, id2, conflict);
	inputs[9] = temp_results[0];
	id1_c += temp_results[1];
	id2_c += temp_results[2];

	outputType(node.dup_nonConf, temp_results, id1, id2, conflict);
	inputs[10] = temp_results[0];
	id1_c += temp_results[1];
	id2_c += temp_results[2];

	outputType(node.dup_unknownConf, temp_results, id1, id2, conflict);
	inputs[11] = temp_results[0];
	id1_c += temp_results[1];
	id2_c += temp_results[2];

	inputs[12] = std::min(id1_c,id2_c);
	inputs[13] = std::max(id1_c,id2_c);
	inputs[14] = (id1_c+id2_c)/2.0;
}

int ICBSSearch::record_ones()
{
	ifstream infile;
...
void ICBSSearch::chooseConflict(ICBSNode &node)
{
	...
	num_steps ++;
	int pre_num = record_ones();
	node.dup_num = pre_num;

	node.dup_rectSemiConf = node.rectSemiConf;
	node.dup_rectNonConf = node.rectNonConf;
	node.dup_cardinalConf = node.cardinalConf;
	node.dup_semiConf = node.semiConf;
	node.dup_nonConf = node.nonConf;
	node.dup_unknownConf = node.unknownConf;
	node.dup_Ncollisions = node.num_of_collisions;

	float prob = 0.3;
	float rand_p = (float)rand()/RAND_MAX;
	//std::cout << rand_p << std::endl;
	...
	if (rand_p < prob)
	{
		...
		if (!node.cardinalConf.empty())
		{
			...
			{
				generate_inputs(node, conflict, inputs);
...
bool ICBSSearch::runICBSSearch() 
{
	...
	{
		printResults();
		outputResults(goal_node->parent);
		std::cout << "output finished" << std::endl;
		std::cout << "cost: " << goal_node->depth << std::endl;
		output_cost(static_cast<int>(goal_node->depth));
	}

	return solution_found;
}

void ICBSSearch::output_cost(int num)
{
	ofstream outfile;
...
void ICBSSearch::outputResults(ICBSNode *curr)
{
	ofstream outfile;
	outfile.open ("trace.txt");
	ICBSNode *curr_node = curr;
	...
	std::vector<double> inputs;
	double output;
	int max_dp = static_cast<int>(curr->depth) + 1;
	int final_num = curr->dup_num + 1;

	while (curr_node->parent != nullptr)
	{
		//outfile << get<0>(*curr_node->conflict) << " ";
		//outfile << get<1>(*curr_node->conflict) << "\n";
		inputs.clear();
		for (int i = 0; i < 15; i++)
		{
			inputs.push_back(0.0);
		...
		if (get<2>(*curr_node->conflict) < 0)
		{
			outfile << "1 0 0 ";
			outfile << getRectangleTime(*curr_node->conflict, paths, ml->cols) << " ";
			inputs[0] = 1;
			inputs[3] = getRectangleTime(*curr_node->conflict, paths, ml->cols);
		} else if (get<3>(*curr_node->conflict) < 0)
		{
			outfile << "0 1 0 ";
			outfile << get<4>(*curr_node->conflict)<< " ";
			inputs[1] = 1;
			inputs[3] = get<4>(*curr_node->conflict);
		} else {
			outfile << "0 0 1 ";
			outfile << get<4>(*curr_node->conflict)<< " ";
			inputs[2] = 1;
		...
		inputs[3] = inputs[3]/10.0;
		outfile << curr_node->dup_Ncollisions << " " << curr_node->depth <<  " "; //<< " " << curr_node->num_of_collisions
		inputs[4] = curr_node->dup_Ncollisions;
		inputs[5] = static_cast<int>(curr_node->depth);

		std::vector<int> temp_results;
		int id1 = get<0>(*curr_node->conflict), id2 = get<1>(*curr_node->conflict);
		int type[6];
		int id1_c = 0, id2_c = 0;

		outputType(curr_node->dup_rectSemiConf, temp_results, id1, id2, curr_node->conflict);
		type[0] = temp_results[0];
		inputs[6] = temp_results[0];
		id1_c += temp_results[1];
		id2_c += temp_results[2];

		outputType(curr_node->dup_rectNonConf, temp_results, id1, id2, curr_node->conflict);
		type[1] = temp_results[0];
		inputs[7] = temp_results[0];
		id1_c += temp_results[1];
		id2_c += temp_results[2];

		outputType(curr_node->dup_cardinalConf, temp_results, id1, id2, curr_node->conflict);
		type[2] = temp_results[0];
		inputs[8] = temp_results[0];
		id1_c += temp_results[1];
		id2_c += temp_results[2];

		outputType(curr_node->dup_semiConf, temp_results, id1, id2, curr_node->conflict);
		type[3] = temp_results[0];
		inputs[9] = temp_results[0];
		id1_c += temp_results[1];
		id2_c += temp_results[2];

		outputType(curr_node->dup_nonConf, temp_results, id1, id2, curr_node->conflict);
		type[4] = temp_results[0];
		inputs[10] = temp_results[0];
		id1_c += temp_results[1];
		id2_c += temp_results[2];

		outputType(curr_node->dup_unknownConf, temp_results, id1, id2, curr_node->conflict);
		type[5] = temp_results[0];
		inputs[11] = temp_results[0];
		id1_c += temp_results[1];
		id2_c += temp_results[2];
		...
		outfile << std::min(id1_c,id2_c) << " " << std::max(id1_c,id2_c) << " " << (id1_c+id2_c)/2.0 << "\n";

		//outfile << max_dp - static_cast<int>(curr_node->depth) << "\n";
		outfile << final_num - curr_node->dup_num << "\n";
		...
		//output = max_dp - static_cast<int>(curr_node->depth);
		output = (final_num - curr_node->dup_num)/10.0;
		//std::cout << final_num << std::endl;

		mlp.update(inputs, output);
...
void ICBSSearch::outputType(std::list<std::shared_ptr<Conflict>> &conflictList, std::vector<int> &results, int agent1, int agent2, std::shared_ptr<Conflict> chosen)
{
	results.clear();
	...
	for (std::shared_ptr<Conflict> conflict: conflictList)
	{
		if (chosen == conflict)
		{
			results[0] = 1;
		}
		if (get<0>(*conflict) == agent1)
		{
			results[1] = results[1] + 1;
		}
		if (get<0>(*conflict) == agent2)
		{
			results[2] = results[2] + 1;
		}
		if (get<1>(*conflict) == agent1)
		{
			results[1] = results[1] + 1;
		}
		if (get<1>(*conflict) == agent2)
		{
			results[2] = results[2] + 1;
		}
	}
